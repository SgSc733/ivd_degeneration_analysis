...
def wide_to_disc_long(
    ...
    *,
    ...
    if not frames:
        raise ValueError("未在特征列名中识别到任何节段（如 _L3-L4_）。")
...
def _read_pfirrmann_table(path: str | Path) -> pd.DataFrame:
    ...
    if p.suffix.lower() == ".xlsx":
        return read_xlsx_sheet_to_df(p)
    if p.suffix.lower() == ".xls":
        try:
            return pd.read_excel(p)
        except ImportError as exc:
            raise ImportError(
                "Reading legacy .xls label files requires an Excel engine installed in the target environment. "
                "Use .xlsx to enable the bundled stdlib reader."
            ) from exc
...
def save_analysis_result(
    ...
    *,
    ...
    try:
        from utils.robustness_plots import save_robustness_figures
