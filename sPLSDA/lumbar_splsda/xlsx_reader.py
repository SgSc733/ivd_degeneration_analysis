...
Why: the target conda env may not ship with openpyxl, but we still need to read
`pfirr_data.xlsx` (small, simple sheet) without extra dependencies.

This supports the common subset:
- shared strings (t="s")
...
def _sheet_name_to_path(zf: zipfile.ZipFile) -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    # workbook.xml uses a different namespace for r:id; just inline the full URI.
    ...
    for sh in wb.findall("main:sheets/main:sheet", _NS):
        ...
        if not (name and target):
            continue
        # `Target` can be either relative (e.g. "worksheets/sheet1.xml") or absolute
        # (e.g. "/xl/worksheets/sheet1.xml"). Normalize to the zip-internal path.
...
def _parse_sheet_matrix(
    ...
    for row in sheetData.findall("main:row", _NS):
        ...
        for c in row.findall("main:c", _NS):
            ...
            t = c.attrib.get("t")
            # Cell value decoding (minimal subset).
            #
            # - shared strings: t="s", value stored in <v> as an index into xl/sharedStrings.xml
            # - inline strings: t="inlineStr", value stored under <is><t>...</t></is>
            # - numeric/default: value stored in <v>
            if t == "inlineStr":
                is_el = c.find("main:is", _NS)
                if is_el is None:
                    val = None
                else:
                    texts = [te.text or "" for te in is_el.findall(".//main:t", _NS)]
                    val = "".join(texts)
            else:
...
def read_xlsx_sheet_to_df(xlsx_path: str | Path, sheet_name: str | None = None) -> pd.DataFrame:
    """
    Read a sheet from .xlsx into a DataFrame.
    ...
    header_row = 0
    while header_row < len(mat):
        row = mat[header_row]
        if any((x is not None and str(x).strip() != "") for x in row):
            break
        header_row += 1
    if header_row >= len(mat):
        return pd.DataFrame()

    header = [str(x).strip() if x is not None else "" for x in mat[header_row]]
    rows = mat[header_row + 1 :]
