...
def save_explain_stability_plots(
    *,
    ...
    """Shape function stability: fold curves + mean±SD band for top features."""
    _ensure_dir(out_dir)
    selected = list(importance["feature"].head(int(top_n)).tolist())
    for feat in selected:
        available = [fd for fd in folds if feat in fd.feature_names and feat in fd.X_raw.columns]
        if not available:
            continue
        xs: list[np.ndarray] = []
        ys: list[np.ndarray] = []
        ...
        for k, (x_u, y_u) in enumerate(zip(xs, ys)):
            ax.plot(x_u, y_u, color="tab:gray", alpha=0.25, linewidth=1.0, label=None if k else "Fold curves")
