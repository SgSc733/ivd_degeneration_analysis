...
# NOTE: Vendored into this workspace to keep threshold calibration consistent with the main plan.
import argparse
import json
import sys
...
if str(_REPO_ROOT) not in sys.path:
    # Allow running this script from anywhere while still importing `lumbar_splsda.*`.
...
def _predict_from_p_gt(p_gt: np.ndarray, thr_vec: np.ndarray) -> np.ndarray:
    # CORAL decode: y_pred = 1 + sum_k I(p_gt_k > thr_k)
...
def _parse_threshold_matrix(
    *,
    ...
    # Parse config.ordinal.decision_threshold -> (n_tasks, K-1) threshold matrix.
    K = int(n_classes)
    n_bounds = int(K - 1)
...
def _thr_cols(*, thr: np.ndarray, n_thr_cols: int, placeholder: float) -> dict[str, float]:
    # Format threshold columns for output CSV (always at least thr1..thr5 for comparability).
...
def _metrics_to_dict(m: Any) -> dict[str, float]:
    # Convert `lumbar_splsda.metrics.OrdinalMetrics` (or a compatible object) to a JSON-friendly dict.
    return {
        "mae": float(m.mae),
...
def calibrate_and_apply(
    *,
    ...
    from lumbar_splsda.metrics import compute_ordinal_metrics  # local import to keep CLI fast when apply=False
