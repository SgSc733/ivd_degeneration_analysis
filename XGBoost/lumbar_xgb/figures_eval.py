...
"""Generate evaluation figures for XGBoost baseline, aligned with ProtoNAM outputs.

All prediction-based figures MUST be based on the *calibrated* decoded discrete grade
(i.e., using decision_thresholds_calibrated.json produced from pooled OOF).
"""
...
from lumbar_xgb.figures_eval_calibration import (
    save_reliability_diagram,
    save_reliability_diagram_by_class,
    save_reliability_diagram_cumulative,
)
from lumbar_xgb.figures_eval_consistency import lin_ccc, save_bland_altman
from lumbar_xgb.figures_eval_plots import (
...
def _metrics_by_task_calibrated(
    *,
    df_cal: pd.DataFrame,
    n_classes: int,
    task_names: list[str],
    ...
    for t, name in enumerate(list(task_names)):
        mask = pd.to_numeric(df_cal["task"], errors="raise").astype(int).to_numpy() == int(t)
        if int(mask.sum()) == 0:
            continue
        m = compute_ordinal_metrics(
            y_true=df_cal.loc[mask, "y_true"].to_numpy(),
            y_pred=df_cal.loc[mask, "y_pred_calibrated"].to_numpy(),
            y_cont=df_cal.loc[mask, "y_cont"].to_numpy(),
...
def generate_eval_figures(*, run_dir: str | Path) -> dict[str, Any]:
    ...
    # Fold-level figures (overwrite fold_dir confusion/scatter).
    ...
    for fold_dir in fold_dirs:
        ...
        df = pd.read_csv(p, encoding="utf-8")
        df_cal = apply_calibrated_thresholds(df=df, thr=thr, out_col="y_pred_calibrated")
        # Persist calibrated discrete grades for traceability (do NOT delete original y_pred).
        df_cal.to_csv(fold_dir / "val_predictions_calibrated.csv", index=False, encoding="utf-8")
        try:
            by_task_cal = _metrics_by_task_calibrated(df_cal=df_cal, n_classes=K, task_names=thr.task_names)
            if not by_task_cal.empty:
                by_task_cal.to_csv(fold_dir / "val_metrics_by_task_calibrated.csv", index=False, encoding="utf-8")
        except Exception:
            # Metrics-by-task is a helpful artifact but should not block figure generation.
            pass
        save_confusion_matrix(
            y_true=df_cal["y_true"].to_numpy(),
            y_pred=df_cal["y_pred_calibrated"].to_numpy(),
    ...
    by_fold = compute_fold_metrics_calibrated(thr=thr, fold_dirs=fold_dirs)
    ...
    if not miss_prob:
        ...
        # Optional: per-class and cumulative-boundary calibration curves.
        try:
