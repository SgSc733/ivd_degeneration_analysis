...
# 示例：
#   & D:/anaconda3/envs/pnam/python.exe e:/XGBoost/tools/calibrate_decision_thresholds.py --run-dir e:/XGBoost/outputs_xgb/run_YYYYMMDD_HHMMSS --apply
...
if str(_REPO_ROOT) not in sys.path:
    # Allow running this script from anywhere while still importing `lumbar_xgb.*`.
    ...
    for _ in range(int(n_iter)):
        for k in range(thr.shape[0]):
            ...
            for cand in grid.tolist():
                ...
                acc = float(np.mean(y_true_i == np.asarray(yp).astype(int))) if len(yp) else 0.0
                qwk = _safe_qwk(y_true, yp)
                mae = float(mean_absolute_error(y_true, yp))
                # Primary objective: strict accuracy (exact match).
                # Tie-breakers: QWK, then lower MAE (stable when many thresholds share the same acc).
                score = (acc, qwk, -mae)
...
def _metrics_to_dict(m: Any) -> dict[str, float]:
    # Convert `lumbar_xgb.metrics.OrdinalMetrics` (or a compatible object) to a JSON-friendly dict.
    return {
        "mae": float(m.mae),
...
def calibrate_and_apply(
    *,
    ...
    from lumbar_xgb.metrics import compute_ordinal_metrics  # local import to keep CLI fast when apply=False
...
def main() -> int:
    ...
    ap.add_argument("--run-dir", required=True, help="e.g. outputs_xgb/run_YYYYMMDD_HHMMSS")
