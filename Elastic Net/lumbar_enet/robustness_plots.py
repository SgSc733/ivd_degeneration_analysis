...
if TYPE_CHECKING:  # pragma: no cover
    from lumbar_enet.feature_selection_legacy import RobustnessAnalysisResult

# Prefer fonts that can render Chinese on Windows; fall back gracefully.
# (Set explicitly to avoid noisy "missing glyph" warnings on common setups.)
...
def _save_figure(fig: Figure, path: Path, *, dpi: int = 200) -> None:
    ...
    # Ensure a canvas exists (Agg) for deterministic headless rendering.
    FigureCanvas(fig)
...
def save_icc_hist_kde(
    ...
    *,
    ...
    if kde is not None:
        xs, ys = kde
        # Scale KDE roughly to histogram count scale for visual overlay.
        # (Avoid dependency on seaborn; this is for quick inspection/QC.)
        bin_width = (float(np.nanmax(v)) - float(np.nanmin(v))) / max(int(bins), 1)
        ax2 = ax.twinx()
        ax2.plot(xs, ys, color="#f58518", linewidth=2.0, alpha=0.95)
        ax2.set_ylabel("KDE density")
        if np.isfinite(bin_width) and bin_width > 0:
            ax2.set_ylim(0, float(np.nanmax(ys)) * 1.15)
...
def save_corr_heatmap(
    ...
    *,
    ...
    n = int(c.shape[0])
    # Dynamic sizing; keep an upper bound for readability.
...
def save_robustness_figures(
    ...
    *,
    ...
    # Prefer PyRadiomics features when available (as requested).
    ...
    if grades_long is not None and statistics_df is not None:
        # Optional cohort/label plots: enabled only when statistics.csv is provided.
