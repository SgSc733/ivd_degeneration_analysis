...
from lumbar_enet.xlsx_reader import read_xlsx_sheet_to_df
...
def _normalize_pooling_feature_types(feature_types: list[str] | None) -> list[str]:
    ...
    If feature_types is None, default to ProtoNAM's historical behavior:
    ["classic", "pyr"].
    """
...
def _attach_intra_patient_pooling(
    ...
    *,
    ...
    """
    Intra-patient statistical pooling (方案1.md 4.2 / ProtoNAM data.pooling).

    Notes:
    - Pooling is computed *within patient* only. With GroupKFold grouped by patient,
      this does not introduce cross-fold leakage.
...
def load_model_input_csv(
    ...
    *,
    ...
    - X: numeric features (no segment one-hot; segment info is in meta)
    ...
    # NOTE: the workspace CSV is UTF-8 (no BOM). Keep encoding configurable to be robust.
    ...
    # IMPORTANT:
    # Do NOT impute missing values here, because this runs BEFORE GroupKFold splitting.
    # Missing values (if any) must be imputed fold-inner (training-only) to avoid leakage.
    ...
    # Segment task index (used for segment-wise evaluation/plots).
...
def attach_pfirrmann_from_xlsx(
    ...
    *,
    ...
    for i in range(len(loaded.meta)):
        ...
        if not col:
            missing_ids.append(str(loaded.meta.loc[i, id_col]))
