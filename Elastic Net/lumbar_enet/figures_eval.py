...
from lumbar_enet.figures_eval_calibration import (
    save_reliability_diagram,
    save_reliability_diagram_by_class,
    save_reliability_diagram_cumulative,
)
from lumbar_enet.figures_eval_consistency import lin_ccc, save_bland_altman
from lumbar_enet.figures_eval_plots import (
...
class CalibratedThresholds:
    """Decision thresholds aligned with task indices."""
...
def apply_calibrated_thresholds(
    *,
    ...
    """Decode y_cont using calibrated per-task thresholds."""
    K = int(thr.n_classes)
    ...
    y_hat = 1 + (y_cont[:, None] > bounds).sum(axis=1).astype(int)
...
def generate_eval_figures(*, run_dir: str | Path) -> dict[str, Any]:
    ...
    # Probability calibration / reliability (optional; enabled if p_cls_* exists).
        ...
        if all(c in oof_df.columns for c in pgt_cols):
            try:
                by_bound = save_reliability_diagram_cumulative(
                    ...
                    title="Probability calibration of cumulative outputs P(y>k) (OOF)",
                    n_bins=10,
                )
