...
def _sheet_name_to_path(zf: zipfile.ZipFile) -> dict[str, str]:
    ...
    for sh in wb.findall("main:sheets/main:sheet", _NS):
        ...
        if not (name and target):
            continue
        # Some workbooks use absolute Targets like "/xl/worksheets/sheet1.xml".
        # Normalize to zip-internal paths without a leading slash.
        ...
        for c in row.findall("main:c", _NS):
            ...
            if t == "inlineStr":
                ...
                if is_el is None:
                    ...
                    texts: list[str] = []
                    for tt in is_el.findall(".//main:t", _NS):
