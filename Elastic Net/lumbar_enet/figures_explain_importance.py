...
def compute_feature_importance(
    *,
    ...
    for fd in folds:
        if fd.f_centered.shape[0] != tasks.shape[0]:
            raise ValueError(f"tasks length mismatch for fold {fd.fold}: {tasks.shape[0]} vs {fd.f_centered.shape[0]}")
        w_sample = fd.w_last[tasks]  # (n_samples, n_feat)
        g = fd.f_centered * w_sample
...
def save_feature_importance_overview(
    *,
    ...
    out_path: Path,
    top_n: int = 20,
    title: str = "Global feature importance (task-weighted contributions)",
) -> None:
