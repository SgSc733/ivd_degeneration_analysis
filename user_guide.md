...
3.  **稳健性相关性分析模块**: 按“Pfirrmann 相关性预筛（Spearman）→ 扰动鲁棒性筛选（ICC）→ Spearman 去冗余”的流程，从海量特征中筛选出既相关、又稳健、且信息不冗余的“黄金特征集”。
...
本系统采用模块化设计，通过统一的GUI界面调度三大核心功能，形成一个完整的数据分析工作流。
  ...
  * **稳健性相关性分析**: 输入未扰动特征表 + 扰动后特征表（可选 Pfirrmann 分级表），输出最终筛选出的稳健特征列表和详细的分析报告。
...
### III. 稳健性相关性分析模块
  ...
  - **相关性预筛**: 使用 Spearman 相关性检验筛选与 Pfirrmann 分级显著相关的特征（可选，需提供分级表）。
  ...
  - **交互式可视化**: 提供ICC分布查看等可视化工具，辅助分析和决策。
...
$$
*   **椎间盘高度 ($\text{Height}_{\text{disc}}$)**: 采用 `calcu_DHI_512.py` 的面积法（HD）计算等效高度。设椎间盘二值掩码为 $M_D(h,w)\in\{0,1\}$，其像素面积为
    $$
    A_D=\sum_{h}\sum_{w} M_D(h,w).
    $$
    通过上下相邻椎体的角点构造椎间盘左右中点（像素坐标用 $(h,w)$ 表示）：
    $$
    P_L=\frac{P^{(u)}_{LD}+P^{(l)}_{LU}}{2},\qquad
    P_R=\frac{P^{(u)}_{RD}+P^{(l)}_{RU}}{2}.
    $$
    椎间盘小宽度（不含膨出）定义为
    $$
    W_D^{\text{small}}=\|P_R-P_L\|_2.
    $$
    为与参考实现一致，中央区域的分割线由 $P_L,P_R$ 构造。令 $C_0=(h_0,w_0)=\frac{P_L+P_R}{2}$，$\Delta h=|h_L-h_R|$，$\Delta w=|w_L-w_R|$，并取 $q=0.75$。中央比例 $\mu$ 区域的四个分割点（按 $lu,ld,ru,rd$）在 $(h,w)$ 坐标下定义为：
    $$
    \begin{aligned}
    \mu_h &= \frac{\mu}{2},\qquad q_h=\frac{q}{2}.\\
    \text{若 } h_L < h_R:\quad
    P_{lu} &= (h_0-\mu_h\Delta h-q_h\Delta w,\ w_0-\mu_h\Delta w+q_h\Delta h),\\
    P_{ld} &= (h_0-\mu_h\Delta h+q_h\Delta w,\ w_0-\mu_h\Delta w-q_h\Delta h),\\
    P_{ru} &= (h_0+\mu_h\Delta h-q_h\Delta w,\ w_0+\mu_h\Delta w+q_h\Delta h),\\
    P_{rd} &= (h_0+\mu_h\Delta h+q_h\Delta w,\ w_0+\mu_h\Delta w-q_h\Delta h);\\
    \text{否则 } (h_L \ge h_R):\quad
    P_{lu} &= (h_0+\mu_h\Delta h-q_h\Delta w,\ w_0-\mu_h\Delta w-q_h\Delta h),\\
    P_{ld} &= (h_0+\mu_h\Delta h+q_h\Delta w,\ w_0-\mu_h\Delta w+q_h\Delta h),\\
    P_{ru} &= (h_0-\mu_h\Delta h-q_h\Delta w,\ w_0+\mu_h\Delta w-q_h\Delta h),\\
    P_{rd} &= (h_0-\mu_h\Delta h+q_h\Delta w,\ w_0+\mu_h\Delta w+q_h\Delta h).
    \end{aligned}
    $$
    以 $P_{lu}\rightarrow P_{ld}$ 和 $P_{ru}\rightarrow P_{rd}$ 两条分割线裁剪得到 $M_D^{(\mu)}$（沿 $h$ 逐行，分别去除左侧与右侧像素）。
    为降低边缘膨出对面积估计的影响，仅统计中央比例 $\mu$（由参数 `central_ratio` 指定，默认 $\mu=0.8$）的区域面积。中央区域由两条分割线裁剪得到，记裁剪后的掩码为 $M_D^{(\mu)}$，其面积为
    $$
    A_D^{(\mu)}=\sum_{h}\sum_{w} M_D^{(\mu)}(h,w).
    $$
    最终椎间盘等效高度定义为
    $$
    \text{Height}_{\text{disc}}=\frac{A_D^{(\mu)}}{W_D^{\text{small}}}.
    $$
    若 `consider_bulging=True`，则沿直线 $\overline{P_LP_R}$ 向左右扫描至掩码边界得到大宽度 $W_D^{\text{big}}$，并在 `calculate_dwr=True` 时计算
    $$
    DWR=\frac{\text{Height}_{\text{disc}}}{W_D^{\text{big}}}.
    $$
*   **椎体高度 ($\text{Height}_{\text{upper}}$, $\text{Height}_{\text{lower}}$)**: 同样采用面积归一化的等效高度（HV）。设椎体二值掩码为 $M_V(h,w)\in\{0,1\}$，其像素面积为
    $$
    A_V=\sum_{h}\sum_{w} M_V(h,w).
    $$
    在椎体掩码上使用 Shi-Tomasi 角点检测定位 4 个角点，并按“左上、右上、左下、右下”排序，记为 $P_{LU},P_{RU},P_{LD},P_{RD}$（坐标为 $(w,h)$）。计算上缘与下缘宽度：
    $$
    W_{\text{up}}=\|P_{LU}-P_{RU}\|_2,\qquad
    W_{\text{down}}=\|P_{LD}-P_{RD}\|_2,
    $$
    并取平均宽度
    $$
    W_V=\frac{W_{\text{up}}+W_{\text{down}}}{2},
    $$
    则椎体等效高度定义为
    $$
    \text{Height}_V=\frac{A_V}{W_V}.
    $$
    说明：对于 S1 椎体，角点排序使用与腰椎略有不同的判别规则（与参考实现一致）。
    对于 L5-S1 层面，按参考实现采用特例：$DHI_{L5\text{-}S1}=\text{Height}_{\text{disc}}/\text{Height}_{\text{upper}}$（上椎体为 L5，不使用 S1 的椎体高度）。
...
| 特征 | 参数 | 示例值 | 意义与参考文献 |
| :--- | :--- | :--- | :--- |
| **DHI** | `central_ratio` | `0.8` | 计算椎间盘高度时用于裁剪中央区域的比例（默认取中间80%），以减少边缘膨出/边缘效应。 |
...
| **形态学核大小** | 膨胀、腐蚀 | 定义了用于形态学操作的椭圆形结构元素的基础尺寸。值越大，单次操作对掩码边界的改变越显著。 |
| **迭代次数** | 膨胀、腐蚀 | 定义了形态学操作的重复次数。值越大，形态学效果（膨胀或腐蚀）越强烈。 |
...
### 3.1 分析工作流（与 方案1(新).md 4.1 一致）

**Pfirrmann 相关性预筛（Spearman） → 扰动鲁棒性筛选（ICC(2,1) + ICC分层聚类） → 特征间 Spearman 去冗余**

说明：

- 步骤1需要 Pfirrmann 分级表；若未提供，可跳过步骤1，仅做鲁棒性 + 去冗余（便于用示例文件快速验证流程）。
...
- 必含列：`case_id`（病例/患者编号）
- 其余列：特征列。当前命名方式中，列名已包含节段信息，例如 `classic_L1-L2_dhi_dhi`、`Deep_L4-L5_base_axial_max_dim_600`。
- 不包含节段名（如 `_L1-L2_`）的列会被忽略（如需纳入，请先按节段展开为对应列）。
- 行：每行一个病例（`case_id` 仅包含病例号，不含扰动后缀）。

#### (2) 扰动后特征CSV

- 必含列：`case_id`
- 行：每行对应一个（病例，扰动类型）。`case_id` 采用 `{base_case_id}_{perturbation_type}` 命名，例如 `10_dilation`、`10_gaussian_noise`、`10_translation_rotation`。
- 列：与未扰动 CSV 的特征列保持一致（同名、同单位，不再额外追加 `_gold/_noise` 后缀）。

#### (3) Pfirrmann 分级文件（CSV/Excel，可选）

- 支持 wide 格式：每行一个病例（`case_id`），每个节段一列（列名需包含 `L1-L2` 等字串），取值支持 `1-5` 或 `I-V`。
- 示例（见 `examples\data.xlsx`）：列名为 `序号, L12, L23, L34, L45, L5S1`，分别对应 `case_id, L1-L2, L2-L3, L3-L4, L4-L5, L5-S1`。
- 支持 long 格式：三列 `case_id, segment, pfirrmann`，其中 `segment` 为 `L1-L2` 等，`pfirrmann` 取值同上。

### 3.3 计算原理

#### I. Pfirrmann 相关性预筛

在未扰动（gold）条件下，对每个特征计算其与 Pfirrmann 分级的 Spearman 相关系数及 p 值；保留 `p < α` 的显著相关特征，得到候选集。

#### II. 扰动鲁棒性筛选

对每个候选特征，将其在 gold + 多种扰动条件下的重复测量组成矩阵，计算 `ICC(2,1)`（双向随机效应、绝对一致性、单次测量）。
随后将每个特征的 `ICC` 视为一维点，采用 Ward 分层聚类切成 `C` 个簇，选择“平均 ICC 最大”的簇作为鲁棒特征群。

#### III. 特征间相关性去冗余

在鲁棒特征群内部，基于未扰动（gold）条件下的特征值计算 Spearman 相关矩阵。若两特征满足 `|rho| > T_dup`，则移除方差更小的特征，得到最终特征集合。

### 3.4 核心参数设置

| 参数 | 推荐值 | 说明 |
| :--- | :--- | :--- |
| `α` | `0.05` | Spearman 显著性水平（保留 `p < α` 的特征） |
| `C` | `3` | ICC 分层聚类簇数；保留“平均 ICC 最大”的簇 |
| `T_dup` | `0.99` | 去冗余阈值；若 `|rho| > T_dup`，移除方差较小者 |

### 3.5 输出文件说明

- `final_robust_features.csv`: 最终筛选的特征列表（包含 ICC；若启用步骤1则附 Spearman rho/p）。
- `icc_values.csv`: 所有候选特征的 ICC(2,1) 及聚类簇标签。
- `spearman_with_pfirrmann.csv`: （若启用步骤1）所有候选特征与 Pfirrmann 的 Spearman 统计量。
- `analysis_report.txt`: 文本摘要报告（条件数、样本数、各步骤保留数量等）。
------
