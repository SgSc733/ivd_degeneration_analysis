...
def save_case_contribution_plots(
    *,
    ...
    """Typical cases: additive decomposition bar plots (centered contributions)."""
    ...
    # Prefer the real ID column recorded in data_schema.json; fallback to the common default.
    id_col = None
    schema_path = run_dir / "data_schema.json"
    if schema_path.exists():
        try:
            schema = json.loads(schema_path.read_text(encoding="utf-8"))
            id_col = str(schema.get("id_col") or "").strip() or None
        except Exception:
            id_col = None
    if id_col is None:
        id_col = "case_id_层级"
    ...
    for i_case in range(int(cases.shape[0])):
        ...
        feat_names = list(fd.feature_names)
        if fd.f_raw.ndim != 2 or len(feat_names) != int(fd.f_raw.shape[1]):
            raise ValueError(
                f"Fold {fold} contribution shape mismatch: "
                f"{len(feat_names)} feature names for f_raw shape {fd.f_raw.shape}"
            )
        t = int(tasks[orig_idx])

        f_mean = fd.f_mean.astype(float)
        f_i = fd.f_raw[orig_idx, :].astype(float)
        baseline = float(fd.intercept) + float(f_mean.sum())
        contrib = f_i - f_mean
        ...
        ax.invert_yaxis()
        ax.set_xlabel("Contribution to score s_raw (centered)")
        case_id_val = row.get(id_col, orig_idx)
        title = (
            f"Case {i_case+1}: fold={fold}, task={t}, id={case_id_val}\n"
        ...
        case_id = str(case_id_val).replace("/", "_").replace("\\", "_")
