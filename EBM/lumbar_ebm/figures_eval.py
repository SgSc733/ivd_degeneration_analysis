...
from lumbar_ebm.figures_eval_calibration import (
    save_reliability_diagram,
    save_reliability_diagram_by_class,
    save_reliability_diagram_cumulative,
)
from lumbar_ebm.figures_eval_consistency import lin_ccc, save_bland_altman
from lumbar_ebm.figures_eval_plots import (
...
def _extract_class_probs(oof_df: pd.DataFrame, *, n_classes: int) -> tuple[np.ndarray, list[str]] | tuple[None, list[str]]:
    """Return class probability matrix (n,K) and chosen column names, or (None, missing_cols)."""
    K = int(n_classes)
    cols_pcls = [f"p_cls_{k}" for k in range(1, K + 1)]
    cols_classprob = [f"class_prob_{k}" for k in range(1, K + 1)]
    if all(c in oof_df.columns for c in cols_pcls):
        return oof_df[cols_pcls].to_numpy(dtype=float), cols_pcls
    if all(c in oof_df.columns for c in cols_classprob):
        return oof_df[cols_classprob].to_numpy(dtype=float), cols_classprob
    miss = [c for c in cols_pcls if c not in oof_df.columns]
    miss2 = [c for c in cols_classprob if c not in oof_df.columns]
    return None, sorted(set(miss + miss2))
...
def generate_eval_figures(*, run_dir: str | Path) -> dict[str, Any]:
    ...
    probs, miss_prob = _extract_class_probs(oof_df, n_classes=K)
    ...
    return {
        ...
        "missing_prob_cols": list(miss_prob) if probs is None else [],
    }
