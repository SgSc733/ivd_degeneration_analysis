...
💡 提示：参数已设置为常用默认值！
    ...
    'en': {
        'title': 'IVD Degeneration Feature Extraction System',
...
class IntegratedFeatureExtractorGUI:
    ...
    def _setup_other_features(self, parent):
        ...
        ttk.Label(worker_frame, text=f"(CPU核心数: {mp.cpu_count()})").pack(side="left")
    ...
    def extract_other_features(self):
        ...
        try:
            if self.input_type.get() == "single":
                ...
                spacing = list(image.GetSpacing())[::-1]  # SimpleITK使用zyx顺序
