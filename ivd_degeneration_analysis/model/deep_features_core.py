...
def _load_volume_as_zyx(path: str) -> np.ndarray:
...
def extract_disc_features(image_3d, mask_3d, disc_label, disc_name, model, device, 
    ...
    centers = tuple(map(int, center_of_mass(binary_mask_3d)))

    sag_axis = int(np.argmin(image_3d.shape))
    sag_index = int(image_3d.shape[sag_axis] // 2)

    def _take_slice_2d(vol: np.ndarray, axis: int, idx: int) -> np.ndarray:
        if axis == 0:
            return vol[idx, :, :]
        if axis == 1:
            return vol[:, idx, :]
    ...
    img_sagittal = _take_slice_2d(image_3d, sag_axis, sag_index)
    mask_sagittal = _take_slice_2d(binary_mask_3d, sag_axis, sag_index)

    remaining_axes = [ax for ax in (0, 1, 2) if ax != sag_axis]
    axial_axis, coronal_axis = remaining_axes[0], remaining_axes[1]

    axial_index = int(np.clip(centers[axial_axis], 0, image_3d.shape[axial_axis] - 1))
    coronal_index = int(np.clip(centers[coronal_axis], 0, image_3d.shape[coronal_axis] - 1))

    img_axial = _take_slice_2d(image_3d, axial_axis, axial_index)
    mask_axial = _take_slice_2d(binary_mask_3d, axial_axis, axial_index)

    img_coronal = _take_slice_2d(image_3d, coronal_axis, coronal_index)
    mask_coronal = _take_slice_2d(binary_mask_3d, coronal_axis, coronal_index)
...
def extract_deep_features_for_case(image_path, mask_path, config, model_size, agg_strategy, padding_ratio, logger_callback=print):
    ...
    try:
        image_3d_data = _load_volume_as_zyx(image_path)
        mask_3d_data = _load_volume_as_zyx(mask_path)
    except Exception as e:
        logger_callback(f"错误: 无法加载图像/掩码文件: {e}")
        return None

    if image_3d_data.shape != mask_3d_data.shape:
        logger_callback(
            "错误: 图像与掩码尺寸不匹配（请确保二者空间对齐且维度一致）。"
            f" 图像shape={image_3d_data.shape}, 掩码shape={mask_3d_data.shape}"
        )
        return None
