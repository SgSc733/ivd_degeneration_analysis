...
from calculator import (
    ...
    TextureFeaturesCalculator, DSCRCalculator,HIZCalculator,MODICCalculator
)
...
class IntegratedFeatureExtractorGUI:
    def __init__(self, root):
        ...
        self.hiz_calculator = HIZCalculator(**self.config.HIZ_PARAMS, logger_callback=logger_cb, debug_mode=self.show_debug_info.get())
        self.modic_calculator = MODICCalculator(**self.config.MODIC_PARAMS, logger_callback=logger_cb,debug_mode=self.show_debug_info.get())
        ...
        self.enable_other_hiz = tk.BooleanVar(value=True)
        self.enable_other_modic = tk.BooleanVar(value=True)
    ...
    def update_language(self):
        ...
        if hasattr(self, 'other_feature_checkboxes'):
            texts = [
                ...
                self.get_text('enable_hiz'),
                self.get_text('enable_modic')
            ]
    ...
    def _setup_other_features(self, parent):
        ...
        cb9 = ttk.Checkbutton(right_frame, text=self.get_text('enable_hiz'),
                              variable=self.enable_other_hiz)
        cb9.pack(anchor="w", pady=2)
        self.other_feature_checkboxes.append(cb9)

        cb10 = ttk.Checkbutton(left_frame, text=self.get_text('enable_modic'),
                              variable=self.enable_other_modic)
        cb10.pack(anchor="w", pady=2)
        self.other_feature_checkboxes.append(cb10)
    ...
    def _process_single_case_other_features(self, image_path: str, mask_path: str, case_id: str) -> Dict:
            ...
            try:
                ...
                slice_spacing_2d = self._get_slice_spacing_2d(spacing, slice_axis)
                ...
                if self.enable_other_asi.get() or self.enable_other_t2si.get():
                    ...
                    for i, img_slice in enumerate(image_slices):
                        processed_img, processed_mask = self.preprocessor.preprocess_for_signal_intensity(
                            img_slice, mask_slices[i], slice_spacing_2d
                        )
                        ...
                        for level_name, labels in self.config.DISC_LABELS.items():
                            ...
                            for i, img_slice in enumerate(image_slices):
                                edges, processed_mask = self.preprocessor.preprocess_for_fractal(
                                    img_slice, disc_masks_level[i], slice_spacing_2d
                                )
                    ...
                    try:
                        ...
                        for level_name, labels in self.config.DISC_LABELS.items():
                            ...
                            gabor_result = self.gabor_calculator.process_multi_slice(
                                gabor_slices_level,
                                gabor_masks_level,
                                use_parallel=self.enable_parallel.get(),
                            )
                            if not gabor_result:
                                continue
                        ...
                        for level_name, labels in self.config.DISC_LABELS.items():
                            ...
                            for roi_mask in disc_masks_level:
                                binary_mask = self.preprocessor.preprocess_for_shape(
                                    roi_mask, slice_spacing_2d
                                )
                                hu_masks_level.append(binary_mask)

                            hu_result = self.hu_calculator.process_multi_slice(
                                hu_masks_level,
                                hu_masks_level,
                                use_parallel=self.enable_parallel.get(),
                            )
                ...
                if self.enable_other_hiz.get():
                    try:
                        self.log_message("计算HIZ特征...")
                        for level_name, labels in self.config.DISC_LABELS.items():
                            self.log_message(f"  -> 处理 {level_name} HIZ特征...")
                            ...
                            dural_sac_masks_level = []
                            sac_label = getattr(self.config, "DURAL_SAC_LABEL", None)
                            for mask_slice in mask_slices:
                                if sac_label is None:
                                    dural_sac_masks_level.append(np.zeros_like(mask_slice, dtype=np.uint8))
                                else:
                                    dural_sac_mask = (mask_slice == sac_label).astype(np.uint8)
                                    dural_sac_masks_level.append(dural_sac_mask)

                            hiz_features = {}
                            fallback_features = None
                            printed_debug = False
                            printed_return = False
                            for img_slice, disc_mask, sac_mask in zip(
                                    image_slices, disc_masks_level, dural_sac_masks_level
                            ):
                                if not np.any(disc_mask):
                                    continue

                                slice_features = self.hiz_calculator.calculate(img_slice, disc_mask, sac_mask)

                                if fallback_features is None:
                                    fallback_features = slice_features

                                hiz_size = slice_features.get('hiz_size', slice_features.get('hiz_area', 0.0))
                                if hiz_size <= 0:
                                    continue

                                for k, v in slice_features.items():
                                    hiz_features.setdefault(k, []).append(v)

                                if hiz_features:
                                    for k, vals in hiz_features.items():
                                        result[f'classic_{level_name}_hiz_{k}'] = float(np.mean(vals))
                                else:
                                    if fallback_features is None:
                                        fallback_features = {'hiz_size': 0.0, 'num_candidates': 0.0}

                                    for k, v in fallback_features.items():
                                        result[f'classic_{level_name}_hiz_{k}'] = float(v)

                            if not hiz_features:
                                zero = None
                                for img_slice, disc_mask, sac_mask in zip(image_slices, disc_masks_level,
                                                                          dural_sac_masks_level):
                                    if np.any(disc_mask):
                                        zero = self.hiz_calculator.calculate(img_slice, disc_mask, sac_mask)
                                        break

                                if zero is None:
                                    zero = {'hiz_size': 0.0, 'num_candidates': 0.0}

                                result.update({f'classic_{level_name}_hiz_{k}': float(v) for k, v in zero.items()})
                                self.log_message(f"  -> {level_name} 未检测到HIZ候选，已写入0特征")
                                continue

                            hiz_result = {k: np.mean(v) for k, v in hiz_features.items()}
                            result.update({f'classic_{level_name}_hiz_{k}': v for k, v in hiz_result.items()})
                            self.log_message(f"  -> {level_name} 提取了 {len(hiz_result)} 个HIZ特征")

                    except Exception as e:
                        self.log_message(f"❌ HIZ计算失败：{str(e)}")
                        result['hiz_error'] = str(e)

                if self.enable_other_modic.get():
                    try:
                        self.log_message("计算Modic改变...")
                        t2_slices = image_slices

                        t1_slices = None

                        if t1_slices is None:
                            t1_slices = [None] * len(t2_slices)

                        for level_name, labels in self.config.DISC_LABELS.items():
                            self.log_message(f" -> 处理 {level_name} Modic...")
                            ...
                            for mask_slice in mask_slices:
                                upper_mask = (mask_slice == labels['upper']).astype(np.uint8)
                                lower_mask = (mask_slice == labels['lower']).astype(np.uint8)
                                upper_vert_masks.append(upper_mask)
                                lower_vert_masks.append(lower_mask)

                            if (not any(np.any(m) for m in upper_vert_masks)) and (
                            not any(np.any(m) for m in lower_vert_masks)):
                                self.log_message(f" -> 在 {level_name} 未找到椎体掩码, 跳过")
                                continue

                            upper_vert_masks_pp = []
                            lower_vert_masks_pp = []
                            for up_m, low_m, t2 in zip(upper_vert_masks, lower_vert_masks, t2_slices):
                                up_pp = self.preprocessor.preprocess_for_shape(up_m, slice_spacing_2d)
                                low_pp = self.preprocessor.preprocess_for_shape(low_m, slice_spacing_2d)

                                if up_pp.shape != t2.shape:
                                    up_pp = up_m
                                if low_pp.shape != t2.shape:
                                    low_pp = low_m

                                upper_vert_masks_pp.append(up_pp)
                                lower_vert_masks_pp.append(low_pp)

                            modic_regions_level = []
                            for t2, up_mask, low_mask, t1 in zip(t2_slices, upper_vert_masks_pp, lower_vert_masks_pp,
                                                                 t1_slices):

                                if np.any(up_mask):
                                    res_u = self.modic_calculator.calculate(t2, up_mask, t1)
                                    for r in res_u.get('_raw_regions', []):
                                        if r.get('location') == 'lower':
                                            modic_regions_level.append(r)

                                if np.any(low_mask):
                                    res_l = self.modic_calculator.calculate(t2, low_mask, t1)
                                    for r in res_l.get('_raw_regions', []):
                                        if r.get('location') == 'upper':
                                            modic_regions_level.append(r)

                            if not modic_regions_level:
                                self.log_message(f" -> {level_name} 未检测到可用终板区域, 跳过")
                                continue

                            valid_regions = [r for r in modic_regions_level if int(r.get('modic_type', 0)) in (1, 2, 3)]
                            if not valid_regions:
                                self.log_message(f" -> {level_name} 未检测到 Modic 改变(均为0型), 跳过")
                                continue

                            modic_features = {}
                            keys_to_mean = [
                                'modic_mean_intensity',
                                'modic_std_intensity',
                                'modic_intensity_range',
                                'modic_size',
                                'modic_heterogeneity',
                                'modic_lbp_entropy',
                                'modic_mean_gradient',
                            ...
                            for k in keys_to_mean:
                                ...
                                modic_features[k] = float(np.mean(vals)) if vals else 0.0

                            prog = self.modic_calculator._calculate_prognostic_features(valid_regions)
                            for k in ['modic_prognostic_risk', 'modic_inflammatory_activity',
                                      'modic_structural_stability',
                                      'modic_type1_ratio', 'modic_total_regions']:
                                if k in prog:
                                    modic_features[k] = float(prog[k])

                            result.update({f'classic_{level_name}_modic_{k}': v for k, v in modic_features.items()})

                            self.log_message(f" -> {level_name} 提取了 {len(modic_features)}个Modic特征")

                    except Exception as e:
                        self.log_message(f"✗Modic计算失败：{str(e)}")
                        result['modic_error'] = str(e)
    ...
    def _generate_feature_statistics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_types = {
            ...
            'HIZ特征': [col for col in df.columns if col.startswith('hiz_')],
            '终板特征': [col for col in df.columns if col.startswith('modic_')],
    ...
    def _get_slice_spacing_2d(self, spacing_zyx: List[float], slice_axis: int) -> List[float]:
        """
        将 3D 体数据的 spacing_zyx 映射为当前 2D 切片(数组)对应的 spacing。

        约定:
          - numpy 体数据顺序为 (Z, Y, X)
          - _extract_middle_slices 返回的 2D 切片顺序为:
              axis=0 -> (Y, X)
              axis=1 -> (Z, X)
              axis=2 -> (Z, Y)
          - Preprocessor.resample_image 接收 numpy 顺序 spacing（与数组的 row/col 对应）
        """

        if len(spacing_zyx) < 3:
            return list(spacing_zyx)

        z, y, x = spacing_zyx[0], spacing_zyx[1], spacing_zyx[2]
