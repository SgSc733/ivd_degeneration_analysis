...
class DSCRCalculator(BaseCalculator):
    ...
    def calculate(
        ...
        if len(landmark_points) < self.min_landmarks:
            self.logger.warning(
                f"Slice-{slice_idx}: 自动检测到的地标点不足: {len(landmark_points)} < {self.min_landmarks}"
            )
        ...
        epsilon = 1e-6
        m = float(ideal_diameter)
        d = float(actual_diameter)

        ssi = float(dscr) / 100.0

        cea = 0.0
        try:
            sac_centroid = ndimage.center_of_mass(dural_sac_mask)
            disc_centroid = ndimage.center_of_mass(disc_mask)
            centroid_dist = float(np.linalg.norm(np.array(sac_centroid) - np.array(disc_centroid)))
            cea = centroid_dist / (d + epsilon) if d > 0 else 0.0
        except Exception:
            cea = 0.0

        smi = (m - d) / (m + epsilon) if m > 0 else 0.0

        area_disc = float(np.sum(disc_mask > 0))
        area_sac = float(np.sum(dural_sac_mask > 0))
        if area_sac > 0:
            area_ratio = min(area_disc / (area_sac + epsilon), 2.0)
        else:
            area_ratio = 0.0
        dsr = 0.5 * ssi + 0.3 * cea + 0.2 * area_ratio

        irregularity_sac = 0.0
        try:
            contours, _ = cv2.findContours(
                dural_sac_mask.astype(np.uint8),
                cv2.RETR_EXTERNAL,
            ...
            if contours:
                ...
                if perimeter > 0:
                    ...
                    circularity = float(min(max(circularity, 0.0), 1.0))
                    irregularity_sac = 1.0 - circularity
                else:
                    irregularity_sac = 1.0
            else:
                irregularity_sac = 1.0
        except Exception:
            irregularity_sac = 0.0

        nrp = 0.6 * ssi + 0.2 * irregularity_sac + 0.2 * cea

        result = {
            "dscr": float(dscr),
            "actual_diameter_pixels": float(actual_diameter),
            "ideal_diameter_pixels": float(ideal_diameter),
        ...
        self.logger.info(
            f"Slice-{slice_idx}, {disc_level}: DSCR calculated: {dscr:.1f}% "
            f"(d={actual_diameter:.1f}, m={ideal_diameter:.1f})"
        )
    ...
    def _detect_single_vertebra_landmark(
        self,
        ...
        for corner in corners:
            ...
            if 0 <= cy < distance_map.shape[0] and 0 <= cx < distance_map.shape[1]:
                distances.append(distance_map[cy, cx])
            else:
        ...
        midpoint_y = float(np.mean(posterior_corners[:, 1]))
        ...
        sac_right_boundary = int(sac_pixels_on_line.max())
        ...
        x_final = float(np.percentile(filtered_line_x_coords, 99))
        final_y = midpoint_y
    ...
    def _detect_landmarks_auto(self, full_mask: np.ndarray, dural_sac_mask: np.ndarray, slice_idx: int) -> np.ndarray:
        ...
        vertebra_map = Config.DSCR_VERTEBRA_LABELS

        for vert_name, vert_label in vertebra_map.items():
            vertebra_mask = (full_mask == vert_label)
            landmark_yx = self._detect_single_vertebra_landmark(
                vertebra_mask, dural_sac_mask, vert_name, slice_idx
            )
    ...
    def _calculate_ideal_diameter(self, ideal_curve: Tuple, dural_sac_mask: np.ndarray, center_y: int) -> float:
        tck, u = ideal_curve
        ...
        ideal_posterior_x = float(x_curve[closest_idx])
        ...
        actual_posterior_x = float(sac_pixels.max())

        ideal_diameter = abs(actual_posterior_x - ideal_posterior_x)
        return float(ideal_diameter)

    def _calculate_actual_diameter(self, dural_sac_mask: np.ndarray, center_y: int) -> float:
        sac_line = dural_sac_mask[center_y, :]
        sac_pixels = np.where(sac_line > 0)[0]
        if len(sac_pixels) == 0:
            return 0.0

        actual_diameter = float(sac_pixels.max() - sac_pixels.min())
        return float(actual_diameter)
    ...
    def process_multi_slice(
        ...
        final_result = {
            ...
            "ssi": float(np.mean([r["ssi"] for r in all_results])),
            "cea": float(np.mean([r["cea"] for r in all_results])),
            "smi": float(np.mean([r["smi"] for r in all_results])),
            "dsr": float(np.mean([r["dsr"] for r in all_results])),
            "nrp": float(np.mean([r["nrp"] for r in all_results])),
        }
