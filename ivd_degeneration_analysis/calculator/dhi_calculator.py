...
class DHICalculator(BaseCalculator):
    ...
    def calculate(self, upper_vertebra_mask: np.ndarray,
        ...
        try:
            ...
            epsilon = 1e-6
            avg_vh = (upper_vh + lower_vh) / 2.0

            height_ratio = disc_params['disc_height'] / (avg_vh + epsilon)
            width_ratio = disc_params['small_width'] / (disc_params['big_width'] + epsilon)
            msi = height_ratio * width_ratio

            hai = abs(upper_vh - lower_vh) / (avg_vh + epsilon)

            upper_center = np.mean(upper_corners.astype(float), axis=0)
            lower_center = np.mean(lower_corners.astype(float), axis=0)
            center_offset = float(np.linalg.norm(upper_center - lower_center))
            ldi = center_offset / (avg_vh + epsilon)

            actual_disc_area = float(np.sum(np.asarray(disc_mask) > 0))
            ideal_disc_area = float(disc_params['big_width'] * avg_vh)
            dfi = actual_disc_area / (ideal_disc_area + epsilon)
    ...
    def process_multi_slice(self, upper_masks: List[np.ndarray],
        ...
        avg_result = {
            ...
            'msi': np.mean([r['msi'] for r in dhi_results]),
            'hai': np.mean([r['hai'] for r in dhi_results]),
            'ldi': np.mean([r['ldi'] for r in dhi_results]),
            'dfi': np.mean([r['dfi'] for r in dhi_results]),
