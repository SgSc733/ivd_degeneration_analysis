...
class ASICalculator(BaseCalculator):
    ...
    def calculate(self, disc_image: np.ndarray, disc_mask: np.ndarray,
        ...
        try:
            ...
            inflammatory_index = float(skew(disc_intensities))

            sigma_global = float(np.std(disc_intensities))

            tissue_heterogeneity = 0.0
            try:
                covs = gmm.covariances_
                if covs.ndim == 3:
                    comp_sigma = np.sqrt(covs[:, 0, 0]).astype(float)
                elif covs.ndim == 2:
                    comp_sigma = np.sqrt(covs[:, 0]).astype(float)
                else:
                    comp_sigma = np.sqrt(covs).astype(float)

                if len(means) >= 2 and len(comp_sigma) >= 2 and np.all(np.isfinite(comp_sigma)):
                    idx_low = int(np.argmin(means))
                    idx_high = int(np.argmax(means))
                    denom = float(comp_sigma[idx_low] + comp_sigma[idx_high] + epsilon)
                    tissue_heterogeneity = float(abs(means[idx_high] - means[idx_low]) / denom)
                else:
                    tissue_heterogeneity = 0.0
            except Exception:
                tissue_heterogeneity = 0.0

            p25 = float(np.percentile(disc_intensities, 25))
            signal_degeneration_index = float(np.mean(disc_intensities <= p25))

            contrast_preservation_index = float(peak_diff / (sigma_global + epsilon))
            ...
            return {
                ...
                'gmm_data': gmm_data,
                'inflammatory_index': float(inflammatory_index),
                'tissue_heterogeneity': float(tissue_heterogeneity),
                'signal_degeneration_index': float(signal_degeneration_index),
                'contrast_preservation_index': float(contrast_preservation_index),
            }
    ...
    def _prepare_gmm_visualization_data(self, intensities: np.ndarray, gmm: GaussianMixture) -> Dict:
    ...
    def _process_multi_slice_serial(self, image_slices: list, disc_masks: list, csf_masks: list) -> Dict[str, float]:
        ...
        avg_result = {
            ...
            'inflammatory_index': np.mean([r['inflammatory_index'] for r in asi_results]),
            'tissue_heterogeneity': np.mean([r['tissue_heterogeneity'] for r in asi_results]),
            'signal_degeneration_index': np.mean([r['signal_degeneration_index'] for r in asi_results]),
            'contrast_preservation_index': np.mean([r['contrast_preservation_index'] for r in asi_results]),
    ...
    def process_multi_slice(self, image_slices: list, disc_masks: list, csf_masks: list,
    ...
    def process_multi_slice_parallel(self, image_slices: list, disc_masks: list, csf_masks: list) -> Dict[str, float]:
