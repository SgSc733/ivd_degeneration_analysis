...
class T2SignalIntensityCalculator(BaseCalculator):
    ...
    def calculate(self, image: np.ndarray, disc_mask: np.ndarray,
        ...
        prognostic_features = self._calculate_prognostic_features(image, roi_mask, si_ratio)
        result.update(prognostic_features)

        return result

    def _calculate_prognostic_features(self,
                                       image: np.ndarray,
                                       roi_mask: np.ndarray,
                                       si_ratio: float) -> Dict[str, float]:

        roi_pixels = image[roi_mask > 0]
        epsilon = 1e-8
        if roi_pixels.size < 3:
            return {
                'hydration_prognosis_index': 0.0,
                'signal_heterogeneity_risk': 0.0,
                'nucleus_activity_assessment': 0.0,
                'rapid_degeneration_pattern': 0.0
            }

        pixels = roi_pixels.astype(np.float64)

        mean_si = float(np.mean(pixels))
        std_si = float(np.std(pixels))
        hpi = float(si_ratio)

        shr = float(std_si / (mean_si + epsilon))

        homogeneity_factor = float(1.0 / (shr + epsilon))
        naa = float((si_ratio * 0.6) + (homogeneity_factor * 0.4))

        r_low = float(1.0 - min(si_ratio, 1.0))
        ...
        if m2 < epsilon:
            skewness_val = 0.0
            excess_kurtosis_val = 0.0
        else:
            m3 = float(np.mean(diff ** 3))
            m4 = float(np.mean(diff ** 4))
            skewness_val = float(m3 / ((m2 + epsilon) ** 1.5))
            excess_kurtosis_val = float(m4 / ((m2 + epsilon) ** 2) - 3.0)

        r_skew = float(max(skewness_val, 0.0))
        r_kurt = float(1.0 - min(abs(excess_kurtosis_val), 1.0))

        rdp = float(0.5 * r_low + 0.3 * r_skew + 0.2 * r_kurt)

        return {
            'hydration_prognosis_index': hpi,
            'signal_heterogeneity_risk': shr,
            'nucleus_activity_assessment': naa,
            'rapid_degeneration_pattern': rdp
        }
    ...
    def process_multi_slice(self, images: List[np.ndarray],
        ...
        aggregated = {
            ...
            'hydration_prognosis_index': np.mean([r['hydration_prognosis_index'] for r in slice_results]),
            'signal_heterogeneity_risk': np.mean([r['signal_heterogeneity_risk'] for r in slice_results]),
            'nucleus_activity_assessment': np.mean([r['nucleus_activity_assessment'] for r in slice_results]),
            'rapid_degeneration_pattern': np.mean([r['rapid_degeneration_pattern'] for r in slice_results]),
