...
def compute_ordinal_metrics(
    *, y_true: np.ndarray, y_pred: np.ndarray, y_cont: np.ndarray, n_classes: int | None = None
    ...
    # For tiny subsets (e.g. per-task) it is possible to get a constant label/prediction,
    # where kappa is mathematically undefined. We map it to 0.0 to keep downstream summaries stable.
    ...
    # Spearman correlation between ordinal label and continuous output.
    # NOTE:
    # - This warning is NOT about "feature columns being constant".
    # - It happens when the *inputs to correlation* become constant (common for small subsets like per-task metrics,
    #   or early epochs when the model outputs nearly constant predictions).
    if np.unique(y_true).size < 2 or np.unique(y_cont).size < 2:
        spearman = 0.0
    else:
        spearman_val = pd.Series(y_true).corr(pd.Series(y_cont), method="spearman")
        spearman = float(spearman_val) if spearman_val is not None and np.isfinite(spearman_val) else 0.0
