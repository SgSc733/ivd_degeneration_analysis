...
from lumbar_pnam.figures_eval_calibration import (
    save_reliability_diagram,
    save_reliability_diagram_by_class,
    save_reliability_diagram_cumulative,
)
from lumbar_pnam.figures_eval_consistency import lin_ccc, save_bland_altman
from lumbar_pnam.figures_eval_plots import (
...
def generate_eval_figures(*, run_dir: str | Path) -> dict[str, Any]:
    ...
    # Performance stability (discrete predictions per fold).
    by_fold = pd.read_csv(run_dir / "cv_metrics.csv", encoding="utf-8")
    by_fold = by_fold[by_fold["fold"].astype(str).isin(["0","1","2","3","4"])].copy()
    by_fold["fold"] = by_fold["fold"].astype(int)
    if "n" not in by_fold.columns and "n_val" in by_fold.columns:
        by_fold["n"] = by_fold["n_val"]                     # 保险：绘图函数若用到 n
    if not by_fold.empty:
        save_performance_stability_plot(
            by_fold=by_fold,
            out_path=fig_dir / "performance_stability_by_fold.png",
            metrics=["mae","kappa_quadratic","ccc","spearman","acc"],
            title="Performance stability across folds",
        )
    ...
    save_abs_error_distribution(
        ...
        n_classes=K,
        out_path=fig_dir / "abs_error_distance_oof.png",
        title="OOF pooled |y_true - y_pred| distance distribution",
    )
