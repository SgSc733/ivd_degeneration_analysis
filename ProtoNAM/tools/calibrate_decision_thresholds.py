...
# 示例：
#   & D:/anaconda3/envs/pnam/python.exe e:/ProtoNAM/tools/calibrate_decision_thresholds.py --run-dir e:/ProtoNAM/outputs/run_20260211_103925 --apply
import argparse
import json
import sys
...
import numpy as np
import pandas as pd
from sklearn.metrics import cohen_kappa_score, mean_absolute_error, confusion_matrix, f1_score

_REPO_ROOT = Path(__file__).resolve().parents[1]
if str(_REPO_ROOT) not in sys.path:
    # Allow running this script from anywhere while still importing `lumbar_pnam.*`.
...
def _threshold_score(
    *,
    ...
    metrics = {
        ...
        "bacc": _balanced_accuracy(y_true=y_true_i, y_pred=y_pred_i, n_classes=int(n_classes)),
        "qwk": _safe_qwk(y_true_i, y_pred_i),
    ...
    for raw_name in [str(objective), *[str(x) for x in tie_breakers]]:
        ...
        out.append(sign * float(metrics[metric_name]))
    return tuple(out)
    ...
    for _ in range(int(n_iter)):
        for k in range(thr.shape[0]):
            ...
            for cand in grid.tolist():
                thr_c = thr.copy()
                thr_c[k] = float(cand)
                yp = _predict_from_p_gt(p_gt, thr_c)
...
def _thr_cols(*, thr: np.ndarray, n_thr_cols: int, placeholder: float) -> dict[str, float]:
    """
    Format threshold columns for output CSV.

    - If K>6 (i.e., more than 5 boundaries), emit all boundaries (thr1..thr{K-1}).
    - If K<=6, still emit at least thr1..thr5 for easier side-by-side comparison across runs.
      Unused thresholds are filled with `placeholder`.
    """
...
def _metrics_to_dict(m: Any) -> dict[str, float]:
    # Convert `lumbar_pnam.metrics.OrdinalMetrics` (or a compatible object) to a JSON-friendly dict.
    return {
        "mae": float(m.mae),
...
def calibrate_and_apply(
    *,
    ...
    calib_objective = str(objective or tcal_cfg.get("objective", "acc")).strip().lower()
    default_ties = {
        "qwk": ["-mae", "macro_f1"],
        "acc": ["bacc", "qwk", "-mae", "macro_f1"],
        "bacc": ["acc", "qwk", "-mae", "macro_f1"],
    }
    calib_tie_breakers = list(
        tie_breakers if tie_breakers is not None else tcal_cfg.get("tie_breakers", default_ties.get(calib_objective, ["qwk", "-mae"]))
    )
    _threshold_score(
        ...
        tie_breakers=calib_tie_breakers,
    )
    ...
    summary: dict[str, Any] = {
        ...
        "method": "calib_cd_median_apply_all",
        "objective": {"primary": str(calib_objective), "tie_breakers": list(calib_tie_breakers)},
    ...
    from lumbar_pnam.metrics import compute_ordinal_metrics  # local import to keep CLI fast when apply=False
...
def main() -> int:
    ...
    ap.add_argument("--objective", default=None, help="primary metric: acc, bacc, qwk")
    ap.add_argument("--tie-breakers", nargs="*", default=None, help="lexicographic tie-breakers, e.g. bacc qwk -mae")
    ap.add_argument(
        "--apply",
        action="store_true",
    ...
    calibrate_and_apply(
        ...
        tie_breakers=args.tie_breakers,
