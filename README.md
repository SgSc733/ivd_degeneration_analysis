...
本系统采用模块化设计，通过统一的GUI界面调度三大核心功能，形成一个完整的数据分析工作流。
  ...
  * **稳健性相关性分析**: 输入一个包含了“金标准”（未扰动）和多种“扰动后”特征值的数据表，输出最终筛选出的稳健特征列表和详细的分析报告。
...
* **积木块 (Vertebrae / 椎体)**: 这些是坚硬的骨头块。
* **软垫 (Intervertebral Discs / 椎间盘)**: 这些是夹在两块积木之间的、有弹性的椭圆形软垫，起到缓冲和连接的作用。
...
* 接着，您移动到下一个椎间盘（L2-L3），您需要用一个**不同的数字**来代表它，比如数字 **`4`**。那么，L2-L3这个椭圆形区域内的所有像素，其值就都应该是`4`。
* 如果还需要标记脑脊液(CSF)，您可以选择一块脑脊液区域，然后用另一个**独一无二的数字**，比如 **`1`**，来填充它。
...
| 医学结构名称 (Anatomical Structure) | 英文缩写 | **分配的标签值 (Assigned Label Value)** | 备注 (Notes) |
| :--- | :--- | :--- | :--- |
| 脑脊液 | CSF | `1` | 用于信号强度标准化 |
...
1.  **选择工具**: 在软件中找到设置“涂色”数值的工具（通常叫做 "Label" 或 "Paintbrush Value"）。
2.  **标注L1-L2椎间盘**:
